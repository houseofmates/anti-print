...
(function(){
				...
				js: [{
					code: `(function(settingsData){
						if (typeof require !== "undefined"){
							...
							console.warn(
								"[anti-print] invalid content script order: require not defined at",
	...
	settings.onloaded(function(){
		...
		browser.runtime.onMessage.addListener(async function(data){
			...
			if (data["antiprint-new-domain-rnd"]){
				persistentRndStorage.setDomainData(
					data["antiprint-new-domain-rnd"].domain,
					data["antiprint-new-domain-rnd"].incognito,
					data["antiprint-new-domain-rnd"].rnd
				);
				if (keys.length === 1){
					return;
				}
			}
			if (data["antiprint-clear-domain-rnd"]){
				persistentRndStorage.clear(data["antiprint-clear-domain-rnd"] === "force");
				if (keys.length === 1){
					return;
				}
			}
			if (data["antiprint-clear-container-rnd"]){
				persistentRndStorage.clearContainerData(data["antiprint-clear-container-rnd"]);
				if (keys.length === 1){
					return;
				}
			}
		...
		browser.runtime.onConnect.addListener(function(port){
			...
			port.onMessage.addListener(function(data){
				if (data.hasOwnProperty("antiprint-notify")){
					notification.show(port.sender.tab.id, url, data["antiprint-notify"].api);
				}
				if (data.hasOwnProperty("antiprint-clear-page-action")){
	...
	browser.runtime.onInstalled.addListener(function(details){
		...
		switch (details.reason){
			case "install":
				logging.message("anti-print installed");
			...
			case "update":
				settings.onloaded(function(){
					if (!settings.dontShowOptionsOnUpdate){
						logging.message("anti-print updated");
						openOptions(details.reason);
					}
				});
		}
	...
	if (browser.runtime.onSuspend){
		browser.runtime.onSuspend.addListener(async function(){
			logging.message("Suspending anti-print");
			(await browser.tabs.query({})).forEach(function(tab){
				browser.tabs.sendMessage(tab.id, {
					"antiprint-unload": true
				});
			});
		});
	}
