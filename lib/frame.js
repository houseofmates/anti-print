...
(function(require){
	...
	if (window === window.top){
		...
		port.postMessage({"antiprint-clear-page-action": true});
	}
	let tabId;
	...
	function notify(data){
		if (!settings.ignoredAPIs[data.api]){
			...
			if (!sentAPIs[data.api]){
				...
				port.postMessage({"antiprint-notify": data});
			}
		}
	}
	...
	function interceptWindow(window){
		...
		const antiprintData = wrappedWindow.matchMedia(extensionSecret[0]);
		if (antiprintData.secret === extensionSecret[1]){
			if (wrappedWindow.top === wrappedWindow){
				antiprintData.undoIntercept(extension.extensionID);
			}
			else {
	...
	extension.message.on(function(data){
		...
		if (
			data.hasOwnProperty("antiprint-sendNotifications") &&
			data["antiprint-sendNotifications"] === tabId
		){
			logging.notice("sending notifications:", notifications);
			extension.message.send({
				sender: tabId,
				url: window.location.href,
				"antiprint-notificationCounter": notificationCounter,
				"antiprint-notifications": notifications
			});
