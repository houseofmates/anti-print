...
(function(){
	...
	const getPersistentRnd = (function(){
		...
		extension.message.on(function(data){
			if (data["antiprint-set-domain-rnd"]){
				const {domain, incognito, rnd} = data["antiprint-set-domain-rnd"];
			...
			if (data["antiprint-clear-domain-rnd"]){
				...
				let ownPersistendRnd = data["antiprint-clear-domain-rnd"] !== "force"? persistentRnd[domain]: false;
				persistentRnd = Object.create(null);
				if (ownPersistendRnd){
					persistentRnd[domain] = ownPersistendRnd;
					extension.message.send({
						"antiprint-new-domain-rnd": {
							domain,
